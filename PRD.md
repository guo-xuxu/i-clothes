...
- 职责：意图路由、多轮闲聊、体征分析、穿搭推荐、LLM 调用、
  **知识图谱构建与混合检索（RAG，见 `app/knowledge/IMPLEMENTATION.md`）**
...
- [ ] 用户历史记录
- [ ] 收藏功能
- [ ] 向量检索 / 知识图谱（**进行中**：离线抽取 + 建图 + 实体归并 + 向量落盘已实现，
  已入库 12 篇（图 359 节点 / Chroma 359 向量）；在线混合召回待接入；
  实现方案见 `app/knowledge/IMPLEMENTATION.md`）
...
- 季节判断
- 主题穿搭
- **知识图谱 + 向量混合 RAG**（穿搭知识文档 → 图谱/向量落盘 → 在线混合召回增强推荐）
- UI优化
...
**创建时间**：2026-08-11
**负责人**：Claude (PM)
**最后更新**：2026-08-27（v2.1：知识图谱离线构建链路落地——实体归并 + 向量落盘 + 增量构建）
